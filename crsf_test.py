...
def handleCrsfPacket(ptype, data):
    if ptype == PacketsTypes.GPS:
        ...
        print(f"Attitude: Pitch={rad2deg(pitch):0.2f} (deg) Roll={rad2deg(roll):0.2f} (deg) Yaw={rad2deg(yaw):0.2f} (deg)")
...
parser.add_argument('-P', '--port', default='/dev/serial0', required=False, help='Serial port to read from')
parser.add_argument('-b', '--baud', default=420000, required=False, help='Baud rate for the serial port')
parser.add_argument('-i', '--interval', type=float, default=1.0, required=False, help='Interval between attitude commands (seconds)')
...
class TestCRSFController:
    def __init__(self, uart):
        ...
        self.channel_values[RCChannels.THROTTLE] = STICK_MINIMUM  # Throttle starts at lowest position
    ...
    def convert_to_rc_value(self, value):
        ...
        crsf_max = 1976 # 1811
        return int(crsf_min + (crsf_max-crsf_min) * ((value-STICK_MINIMUM)/(STICK_MAXIMUM-STICK_MINIMUM)))
        # 991 -> 1500 (center)
    ...
    def set_channel(self, channel, value):
        ...
        if STICK_MINIMUM <= value <= STICK_MAXIMUM:
            ...
            raise ValueError(f"Channel value must be between {STICK_MINIMUM} and {STICK_MAXIMUM}")
        ...
        while True:
            ...
            if len(input_buffer) > 2:
                ...
                if expected_len > 64 or expected_len < 4:
                    ...
                    # if single_packet[2] == PacketsTypes.ATTITUDE:
                    # print(f"<<< Test received packet: {' '.join([f'{b:02X}' for b in single_packet])}; expected len: {expected_len}") #TODO: remove
